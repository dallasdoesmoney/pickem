            ...
            {links.length > 0 && (
              <div className="flex flex-col gap-2">
                {links.map((link) => {
                  ...
                  return (
                    <div key={link.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
                      <span className="text-white/70 shrink-0">
                        {Icon ? <Icon className="h-5 w-5" /> : <span className="text-base">{def.icon}</span>}
                      </span>
