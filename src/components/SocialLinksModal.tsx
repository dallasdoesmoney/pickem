          ...
          <div className="flex flex-col gap-4">
            {links.length > 0 && (
              <div className="flex flex-col gap-2">
                {links.map((link) => {
                  const def = SOCIAL_PLATFORM_CATALOG[link.platform] ?? { label: link.platform, icon: "🔗", color: "#64748b" };
                  const Icon = SOCIAL_ICON_COMPONENTS[link.platform];
                  return (
                    <div key={link.id} className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2.5">
                      <span className="h-7 w-7 rounded-full flex items-center justify-center shrink-0" style={{ background: def.color }}>
                        {Icon ? <Icon className="h-3.5 w-3.5 text-white" /> : <span className="text-white text-xs">{def.icon}</span>}
                      </span>
