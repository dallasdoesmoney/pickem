...
// Two pools, exactly as src/data/puzzlePlayers.ts builds them - and they
// HAVE to match. A row the app offers that is missing here is rejected on
// guess; a row here the app does not offer is unguessable.
//
//   answerable  the starters from the position files
//   guessable   every QB, RB, WR and TE on a roster, backups included
const starters = readStarters();
const starterIds = new Set(starters.map((m) => m.espnId));
const GUESSABLE_POSITIONS = new Set(["QB", "RB", "WR", "TE"]);
const allPlayers = readAllPlayers();
const full = new Map(allPlayers.map((p) => [p.espnId, p]));
const members = [
  ...starters,
  ...allPlayers.filter((p) => GUESSABLE_POSITIONS.has(p.position)),
];
const source = members.map((m) => {
  ...
  return {
    ...
    answerable: starterIds.has(m.espnId),
    guessable: true,
  };
});
...
--
-- The daily puzzle's pools, as Postgres sees them: ${players.length} guessable,
-- of whom ${players.filter((p) => p.answerable).length} can be the answer.
...
-- 1..${players.filter((p) => p.answerable).length}: most days would find no row at their slot and fall through to
...
-- Expect ${players.filter((p) => p.answerable).length} in rotation and ${players.length} guessable,
...
console.log(
  `  ${players.filter((p) => p.answerable).length} can be the answer` +
    ` (QB/RB/TE starters + ${Math.round(players.filter((p) => p.answerable && p.position === "WR").length / 32)} WR a team, from the position files)`
);
{
  const byPos = new Map();
...
{
  // ANSWERABLE only. A backup with no depth-chart rank is fine - being
  // unranked is most of what makes somebody a backup - and checking all
  // 925 would fail on every roster. The guard exists to keep a player
  // who was picked by roster order out of the ANSWER pool.
  const guessed = players.filter((p) => p.answerable && full.get(p.espnId)?.depthRank == null);
  if (guessed.length) {
    ...
    // The file above is already on disk; this is about whether it should
    // be RUN, which is a different question and worth being exact about.
    console.error(`\n  ${guessed.length} ANSWERS WERE NOT PICKED BY A DEPTH CHART.`);
    console.error(`  The seed file was written - do not load it until these are settled:`);
